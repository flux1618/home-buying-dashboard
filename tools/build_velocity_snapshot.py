...
stream, decompresses it incrementally, and retains only Spartanburg County, Spartanburg metro,
...
COUNTY_FIPS = "45083"
COUNTY_LABEL = "Spartanburg County, SC"
METRO_CODE = "43900"
METRO_GEOGRAPHY_SOURCE = (
    "https://www2.census.gov/programs-surveys/popest/datasets/2020-2022/metro/totals/"
    "cbsa-est2022.csv"
)
...
def _target_identifier(scope: str, row: dict[str, str | None]) -> str | None:
    ...
    particular ZIP or county in the current Market Tracker schema.  Treating it as a FIPS/ZIP
    would silently join unrelated observations.  The published REGION label is therefore
    matched exactly for the one county and parsed strictly for the five-digit ZIP label.
    """
...
def build(*, opener=_open_url, fetched_at: str | None = None) -> dict[str, Any]:
    ...
    # Redfin's public metro object is not available at this download path (403), while
    # Census CBSA 43900 is the Spartanburg County geography in the cited population
    # estimates.  Reuse the published county aggregate only because the scopes coincide;
    # label it explicitly so a boundary change cannot masquerade as a metro observation.
    metro_market = {
        **county_market,
        "label": "Spartanburg, SC (county-equivalent market scope)",
        "source_scope": "county",
    }
    markets = {
    ...
    return {
        ...
        "geography": {
            ...
            "metro_label": "Spartanburg, SC (county-equivalent market scope)",
            "metro_scope_note": (
                "CBSA 43900 is represented by Spartanburg County here; verify this mapping "
                "if OMB delineations change."
            ),
...
def check(snapshot: dict[str, Any], *, today: date | None = None) -> int:
    ...
    if stale:
        ...
        return 1
    print(
        f"{SNAPSHOT.relative_to(REPO)}: Spartanburg metro/county + {len(target_zips)} ZIPs, "
