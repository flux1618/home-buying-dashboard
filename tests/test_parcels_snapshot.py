"""Offline tests for the bounded county parcel snapshot.
...
a request that was not expected fails before it can become a real county request.
"""
...
def attrs(n: int) -> dict:
    return {
        "OBJECTID": n,
        "MAPNUMBER": f"7-17-02-{n:03d}.00",
        "District": "5800",
        "City": "SPARTANBURG",
        "LandUse": "RESIDENTIAL - SINGLE FAMILY",
        "CurrentAssessedLandValue": 1000.00,
        "CurrentAssessedBuildingValue": float(n),
        "SaleDate": 1_726_272_000_000,
        "SaleAmount": 100_000 + n,
    }
...
class TestPagedExtraction:
    def test_reads_all_pages_in_stable_objectid_order(self, builder):
        ...
        assert layer["name"] == "CAMA Parcels"
        assert len(rows) == 2_501
        assert [r["tax_pin"] for r in rows[:2]] == ["7-17-02-001.00", "7-17-02-002.00"]
        assert rows[-1]["assessed_value"] == 3501.0
        assert rows[0]["last_sale_date"] == "2024-09-14"
        assert failures == []
    ...
    def test_mid_run_page_failure_keeps_a_resume_checkpoint_and_refuses_output(self, builder):
        ...
        # A later invocation starts at the checkpoint, not at page zero.  The old rows and
        # the newly fetched rows must meet the count contract before anything is written.
    ...
    def test_a_disappearing_required_field_stops_before_the_count_or_any_page(self, builder):
        transport = CountyTransport(builder, count=1, missing="CurrentAssessedBuildingValue")

        with pytest.raises(builder.SnapshotError, match=r"field\(s\) disappeared.*CurrentAssessedBuildingValue"):
            builder.extract(transport)
...
def test_live_county_layer_still_exposes_the_bounded_snapshot_contract():
    ...
    assert layer["currentVersion"] >= 11
