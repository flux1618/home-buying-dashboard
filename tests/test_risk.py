...
class TestPercentileNotLabel:
    ...
    def test_below_threshold_hazards_raise_no_caveat(self, fake_http, located):
        """Spartanburg wildfire is in the 20s. Silence is the correct output."""
        result = run_with(
            fake_http, "nri_tract", SC_HAZARDS, located, caveat_percentile=90.0
        )

        assert result.values["hazard_wildfire"].value < 50.0
...
class TestNotModeledIsNotLowRisk:
    def test_drought_comes_back_unavailable_not_zero(self, fake_http, located):
        ...
        result = run_with(fake_http, "nri_tract", SC_HAZARDS, located)
    ...
    def test_an_unmodeled_hazard_never_raises_a_caveat(self, fake_http, located):
        ...
        result = run_with(
            fake_http, "nri_tract", SC_HAZARDS, located, caveat_percentile=0.0
        )
...
class TestFailure:
    ...
    def test_no_coordinates_skips_without_a_request(self, fake_http):
        result = RiskStation(hazards=SC_HAZARDS).run(
...
class TestRendering:
    ...
    def test_no_composite_note_when_the_hidden_hazard_is_unremarkable(
        ...
        render_hazards(self.hazards_for(fake_http, "nri_tract", SC_HAZARDS, located))
