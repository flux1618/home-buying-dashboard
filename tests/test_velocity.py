...
def gzip_tsv(builder, *, scope: str, zip_code: str = "29301", renamed: bool = False) -> bytes:
    ...
    for index, period in enumerate(month_starts()):
        ...
        row = {
            ...
            "REGION_TYPE": "zip code" if scope == "zip" else "county",
            # These are source type codes, deliberately not the ZIP/FIPS identifier.
            "REGION_TYPE_ID": "2" if scope == "zip" else "5",
            "PROPERTY_TYPE": "All Residential",
            "IS_SEASONALLY_ADJUSTED": "false",
            "REGION": f"Zip Code: {zip_code}" if scope == "zip" else "Spartanburg County, SC",
            "MEDIAN_DOM": str(20 + index),
...
class GzipTransport:
    ...
    def __init__(self, builder, *, county: bytes | BaseException, zip_rows: bytes | BaseException):
    ...
    def __call__(self, url: str):
        ...
        payload = self.zip_rows if "zip_code" in url else self.county
...
def test_normal_parse_streams_and_reduces_realistic_gzip(monkeypatch):
    ...
    monkeypatch.setattr(builder, "_read_profile_target_zips", lambda: (["29301"], "test"))
    ...
    assert result["markets"]["county"]["45083"]["current"]["median_days_on_market"] == 32
    assert result["markets"]["zip"]["29301"]["current"]["months_of_supply"] == pytest.approx(3.2)
    assert result["markets"]["zip"]["29301"]["year_over_year"]["new_listings"]["absolute_change"] == 12
    assert result["markets"]["zip"]["29301"]["recent_slopes"]["active_inventory"]["12_months"]["direction"] == "increasing"
...
def test_missing_geography_stays_none_and_requests_no_nearby_substitute(tmp_path):
    ...
    result = station.run(Context(address="Unknown, SC 99999", price=1, county_fips="45001"))
...
def test_renamed_upstream_column_is_refused_not_guessed():
    ...
    transport = GzipTransport(builder, county=renamed, zip_rows=renamed)

    with pytest.raises(ValueError, match="columns changed"):
        builder.stream_rows("county", {"45083"}, opener=transport)


def test_truncated_gzip_is_a_clean_source_outage():
    builder = load_builder()
    complete = gzip_tsv(builder, scope="county")
    transport = GzipTransport(builder, county=complete[:-12], zip_rows=complete)
...
def test_stale_snapshot_degrades_instead_of_pretending_market_is_current(tmp_path):
    ...
    result = MarketVelocityStation(path, today=date(2026, 8, 19)).run(
        Context(address="606 Andre Ct, Spartanburg, SC 29301", price=1, county_fips="45083")
    )
...
def test_malformed_snapshot_degrades_under_the_station_never_raise_contract(tmp_path):
    ...
    result = MarketVelocityStation(path).run(Context(address="x 29301", price=1, county_fips="45083"))

    assert result.degradation is not None
    assert "period_covered" in result.degradation.reason


def test_seasonality_never_labels_month_over_month_as_a_trend():
    station = MarketVelocityStation(today=date(2026, 8, 19))
    result = station.run(Context(address="606 Andre Ct, Spartanburg, SC 29301", price=1, county_fips="45083"))
