...
class TestInputsThatMoveTheAnswer:
    ...
    def test_non_owner_occupied_lowers_the_max_price(self, profile):
        """The SC 6% assessment ratio applies. If this ever stops being true, the tax
        module changed and this solver silently started quoting the wrong number."""
...
class TestItDoesNotTouchScoring:
    ...
    def test_compute_is_unchanged_after_a_solve(self, profile):
        first = cost.compute(profile, 310_000, 1650, 2004, 0.0, YEAR)
        solve(profile, dti_ceiling=0.22, sqft=1650)
        second = cost.compute(profile, 310_000, 1650, 2004, 0.0, YEAR)
        assert first == second
...
class TestTheTwoDoorsAgree:
    ...
    Finding a $13,379 "max price" against an $80,000 down payment is how the page's floor
    ...
    def js_solve(self, profile, dti_pct, hoa=0.0):
        ...
        script = (
            self.HARNESS.replace("MILLS", str(snapshot["global"]["tax"]["typical_owner_millage_mills"]))
            .replace("RATIO", str(snapshot["global"]["tax"]["primary_assessment_ratio"]))
            .replace("INS", str(snapshot["global"]["insurance"]["sc_avg_annual"]))
