...
class TestNotModeledIsNotLowRisk:
    ...
    def test_drought_is_modeled_in_a_minority_of_tracts(self, haz):
        """The measured finding this whole two-shape design exists for. If a future NRI
        release models drought everywhere, this fails and the callout copy needs a rewrite
        rather than silently becoming false."""
        drgt = haz["county"]["hazards"]["DRGT"]
        assert drgt["modeled_tracts"] < drgt["total_tracts"]

    def test_where_drought_is_modeled_it_is_high(self, haz):
        drgt = haz["county"]["hazards"]["DRGT"]
        assert drgt["min"] > 50, "the callout claims drought runs high where modeled"
...
class TestPercentilesNotLabels:
    ...
    def test_the_label_inversion_really_exists_in_the_data(self, haz):
        """The callout claims 29301 shows hail 71.0 "Relatively Moderate" above wildfire
        68.9 "Relatively Low". That is a claim about committed data, so it gets asserted
        rather than trusted -- if a refresh changes it, the copy is wrong."""
        z = haz["zips"]["29301"]["hazards"]
        hail, wfir = z["HAIL"], z["WFIR"]
        assert hail["percentile"] > wfir["percentile"]
        assert hail["rating"] != wfir["rating"]
        gap = hail["percentile"] - wfir["percentile"]
        assert gap < 10, "two labels apart on a gap this small is the point being made"
...
class TestProvenance:
    ...
    def test_every_zip_in_the_geojson_got_a_hazard_entry(self, haz):
        expected = {
            f["properties"]["zip"]
            for f in SNAPSHOT["geojson"]["zips"]["features"]
            if f["properties"].get("zip")
        }
