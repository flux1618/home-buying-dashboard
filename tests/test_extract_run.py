...
def test_the_provider_is_handed_redacted_text_and_never_the_original():
    ...
    assert "septic" in Spy.seen.lower()
...
def test_the_offline_provider_extracts_the_fields_that_matter_with_no_key():
    """The deal-breaker inputs, specifically: well/septic drives a hard fail.

    Extraction that produces fields nothing consumes is a demo. These four are read by rules
    that already exist, which is why they were chosen.
    """
    run = go(None)
    found = {f.field_name: f.value for f in run.result.findings}
    assert found["sewer_type"] == "septic"
