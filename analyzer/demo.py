...
def detail(row: dict, profile, year: int) -> None:
    ...
    print(f"    Tax (4%)         ${c['monthly_tax']:>9,.0f}   {DIM}{t['district']}, {t['millage']['primary_mills']:.1f} mills{OFF}")
    print(f"    Insurance        ${c['monthly_insurance']:>9,.0f}   {DIM}SC average — get a quote{OFF}")
    ...
    for key in ("scenario_owner_occupied", "scenario_non_owner_occupied"):
        sc = t[key]
        print(f"    ${sc['annual_tax']:>7,.0f}/yr  {sc['label']:<36} {DIM}{sc['applicable_mills']:.1f} mills{OFF}")
